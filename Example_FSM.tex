...
%Initially, \textsf{current} references the unique \textsf{START} \textsf{State} 
%in the \textsf{FSM} model \textsf{accept} is applied to. An execution step proceeds
%as follows: the \textsf{FSM} reads, or consumes, a \textsf{Word} \textsf{Element}
%\textsf{e}; if there exists an outgoing \textsf{Transition} \textsf{t} with
%a \textsf{Trigger} \textsf{trig} that matches the \textsf{Element} (i.e. 
%$\mathsf{trig} = \mathsf{e}$), the \textsf{current} \textsf{State} is updated to 
%\textsf{t}'s \textsf{tgt}, otherwise \textsf{accept} returns false. When 
%\textsf{w} is empty, \textsf{accept} returns true iff the \textsf{current} 
%\textsf{State} at the end of the execution is \textsf{FINAL}.
...
\begin{description}
   ...
   %briefly appears on top of the fired \textsf{Transition}, then appears inside the
   %updated \textsf{current};
   \item[FSM.2] The token disappears from the \textsf{current} \textsf{State}, and
